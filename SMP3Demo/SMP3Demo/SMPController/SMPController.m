...
- (NSError *)initialiseSMP{
    ...
    self.username = @"jdbcdemo";
    self.password = @"welcome";
    
    // Setup system access info.
    self.smpAppId = @"com.bsil.employee";
    self.smpSecProvider = @"default";
    self.smpServer = @"192.168.60.194";
    self.smpPort = @"8080";
...
{
    NSLog(@"Sucessfully registered user.=%@",userManager);
    NSString* appConID= [userManager applicationConnectionID];
    NSLog(@"appConID=%@",appConID);
    [EncryptionKeyManager resetEncryptionKey];
    
    SMPAppSettings* appSettings = [SMPAppSettings initializeWithConnection:self.clientConn userName:@"username" password:@"password"];
    NSError *error = nil;
    self.endpointURL = [appSettings getApplicationEndpointWithError:&error];
    NSDictionary *settings = [appSettings getConfigPropertyMapWithError:&error];
    NSLog(@"appSettings=%@",settings);
    self.odataURL = [settings valueForKey:keyApplicationSettings_ProxyApplicationEndpoint];
    NSLog(@"self.odataURL=%@",self.odataURL);
    if (!error)
    {
        //Continue processing
        NSLog(@"self.endpointURL=%@",self.endpointURL);
    }
...
- (void) requestServiceDocument{ // Make a request for the service document
    [self requestDataForURL:self.endpointURL andTag:1];
}

- (void) requestMetadata{ // Make a request for the metadata of the service.
    [self requestDataForURL:[self.endpointURL stringByAppendingString:@"$metadata"] andTag:2];
}

- (void) requestData{ // Make a request for the actual data.
    [self requestDataForURL:[self.endpointURL stringByAppendingString:self.entityName] andTag:3];
}

- (void) requestData:(NSString*)query{ // Make a request for the actual data.
    [self requestDataForURL:[self.endpointURL stringByAppendingString:[NSString stringWithFormat:@"%@%@",self.entityName,query]] andTag:4];
}
...
- (void) handleReqSuccess:(Request *)request{
    ...
    switch ([request requestTag]) {
        ...
        case 3: // this is the actual data.
            ...
            break;
        case 4: // this is the actual data.
            [self parseDataRequest:request];
            break;
        default:
            break;
